...
function TRB.Functions.Character:EnableCharacterChange()
	...
	characterChangeFrame:RegisterUnitEvent("UNIT_ENTERED_VEHICLE", "player")
	characterChangeFrame:RegisterUnitEvent("UNIT_EXITED_VEHICLE", "player")
	...
	characterChangeFrame:RegisterUnitEvent("UNIT_FLAGS", "player")
...
---Capabilities a spec declared to Core (see Classes\SpecDescriptor.lua), or nil.
---@param classId integer?
---@param specId integer?
---@return TRB.Classes.SpecDescriptor?
function TRB.Functions.Character:GetSpecDescriptor(classId, specId)
	return TRB.Classes.SpecDescriptor:Get(classId, specId)
end

---Capabilities of the active character's spec, or nil.
---@return TRB.Classes.SpecDescriptor?
function TRB.Functions.Character:GetActiveSpecDescriptor()
	return TRB.Classes.SpecDescriptor:GetActive()
end

---A spec's effective settings by composite key: the specCache's recomposed settings when that spec has
---been filled, otherwise the raw saved settings for it.
---@param compositeKey string?
---@return table?
function TRB.Functions.Character:GetSpecSettingsByKey(compositeKey)
	if compositeKey == nil then
		return nil
	end
	local cache = TRB.Data.specCache and TRB.Data.specCache[compositeKey]
	if cache and cache.settings then
		return cache.settings
	end
	local entry = TRB.Data.specRegistry[compositeKey]
	if entry == nil or TRB.Data.settings == nil then
		return nil
	end
	local classSettings = TRB.Data.settings[entry.className]
	return classSettings and classSettings[entry.specName] or nil
end

---The active character's raw saved spec settings (not recomposed with the global toggles).
---@return table?
function TRB.Functions.Character:GetActiveRawSpecSettings()
	local character = TRB.Data.character
	if character == nil or character.className == nil or character.specName == nil or TRB.Data.settings == nil then
		return nil
	end
	local classSettings = TRB.Data.settings[character.className]
	return classSettings and classSettings[character.specName] or nil
end

---Whether any spec of a class declares shapeshift-form driven display (descriptor `forms`).
---@param classId integer?
---@return boolean
function TRB.Functions.Character:ClassHasForms(classId)
	local classEntry = classId ~= nil and TRB.Data.classRegistryByIds[classId] or nil
	if classEntry == nil then
		return false
	end
	for _, specEntry in ipairs(classEntry.specs) do
		if specEntry.descriptor ~= nil and specEntry.descriptor.forms ~= nil then
			return true
		end
	end
	return false
...
---Whether an options panel for classId/specId edits what the active character displays right now, so
---changes should be applied live. The Global panel (nil ids) is the caller's decision and returns false.
---A class whose display follows shapeshift forms shares live bar state across all of its specs.
---@param classId integer?
---@param specId integer?
---@return boolean
function TRB.Functions.Character:IsPanelForLiveSpec(classId, specId)
	local character = TRB.Data.character
	if classId == nil or specId == nil or character == nil or character.classId ~= classId then
		return false
	end
	if character.specId == specId then
		return true
	end
	local descriptor = self:GetActiveSpecDescriptor()
	return descriptor ~= nil and descriptor.forms ~= nil
end
...
function TRB.Functions.Character:EnsureSpecSettings(className)
	...
	local optionsKey = TRB.Functions.Character:GetClassModuleName(className)
	...
	-- If we just seeded fresh default bar text for this class, let the flavor mark any one-shot bar
	-- text migration as done. Such migrations are meant for users upgrading from an older format,
	-- not for specs that just received brand-new defaults. Without this, logging into the class
	-- later would trigger the migration and clobber any changes the user made to bar text via
	-- the cross-class options panel.
	if seededBarTextForClass and TRB.Flavor.OnBarTextSeeded ~= nil then
		TRB.Flavor.OnBarTextSeeded(settings, className)
	end
