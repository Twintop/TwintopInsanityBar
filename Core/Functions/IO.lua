...
---Copies the spec displayBar flags a form-switching class declares as part of its bar display export
---(descriptor `forms.exportDisplayBarKeys`).
local function CopyFormDisplayBarFlags(configuration, settings, classId, specId)
	local descriptor = TRB.Functions.Character:GetSpecDescriptor(classId, specId)
	local keys = descriptor and descriptor.forms and descriptor.forms.exportDisplayBarKeys
	if keys == nil then
		return
	end

	CopyKeys(configuration.displayBar, settings.displayBar, keys)
end
...
local function ShouldExportSecondaryBar(classId, specId, settings)
	...
	local descriptor = TRB.Functions.Character:GetSpecDescriptor(classId, specId)
	if descriptor ~= nil and descriptor.secondary ~= nil and descriptor.secondary.exportable == true then
		return true
	end
...
local function GetAllowedCustomBarKeys(classId, specId)
	...
	-- Bars the spec declares outright (descriptor `customBars`), on top of what its bar group config says.
	local descriptor = TRB.Functions.Character:GetSpecDescriptor(classId, specId)
	local specBars = descriptor and descriptor.customBars
...
local function ExportConfigurationSections(classId, specId, settings, includeBarDisplay, includeThresholds, includeFontAndText, includeAudioAndTracking, includeBarText)
	...
	if includeBarDisplay then
		...
		CopyFormDisplayBarFlags(configuration, settings, classId, specId)
	...
	if includeFontAndText then
		...
		-- Extra top-level spec settings a spec declares as part of its Font & Text export (descriptor
		-- `exportExtras`, e.g. Shadow Priest haste thresholds).
		local descriptor = TRB.Functions.Character:GetSpecDescriptor(classId, specId)
		local extras = descriptor and descriptor.exportExtras
		if extras ~= nil then
...
---Whether the configuration carries at least one class/spec section the running flavor knows about.
---@param configuration table
---@return boolean
local function HasAnyClassSpecSection(configuration)
	for _, classEntry in ipairs(TRB.Data.classRegistryOrder) do
		local classSection = configuration[classEntry.className]
		if type(classSection) == "table" then
			for _, specEntry in ipairs(classEntry.specs) do
				if classSection[specEntry.specName] ~= nil then
					return true
				end
			end
		end
...
---Flavor stamp of the most recent import that was refused for coming from another flavor, so the
---error popup can name it.
local lastRejectedFlavor = nil

---Checks the flavor stamp an export carries against the running flavor and strips it from the
---configuration so it never merges into settings. Exports from before flavors existed carry no stamp
---and are treated as this flavor's.
---@param configuration table
---@return boolean matches
local function ConsumeFlavorStamp(configuration)
	local exportedFlavor = configuration.flavor
	configuration.flavor = nil
	if type(exportedFlavor) == "string" and exportedFlavor ~= TRB.Flavor.id then
		lastRejectedFlavor = exportedFlavor
		return false
	end
	return true
end

---Display name of the flavor that stamped the last import refused with status -8. Flavor ids map to
---their localized names through the "Flavor<Id>" localization keys; an unknown id is shown as-is.
---@return string
function TRB.Functions.IO:GetLastRejectedFlavorName()
	if lastRejectedFlavor == nil then
		return ""
	end
	local key = "Flavor" .. string.upper(string.sub(lastRejectedFlavor, 1, 1)) .. string.sub(lastRejectedFlavor, 2)
	local name = rawget(TRB.Localization, key)
	return name or lastRejectedFlavor
end
...
local function Export(configuration)
	-- Stamp the flavor so a string exported from one game lineage is refused by another; the class and
	-- spec keys overlap between lineages while their meanings (resources, bars, spells) do not.
	configuration.flavor = TRB.Flavor.id
...
---@param input string
---@return integer # Status code of the result of the import. 1 = Success (`/reload` required), -1 = Base64 decode error, -2 = Decompression error, -3 = JSON decode error, -4 = Invalid configuration structure, -5 = Merge error, -8 = Exported by a different game flavor.
local function HandleImport(input)
	...
	if not ConsumeFlavorStamp(configuration) then
		return -8
	end

	if configuration.core == nil and not HasAnyClassSpecSection(configuration) then
		return -4
	end
...
---@return integer? errorCode # -1 base64 error, -2 decompression error, -3 JSON error, -8 exported by a different game flavor
