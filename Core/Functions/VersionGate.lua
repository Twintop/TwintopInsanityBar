...
local versionBlocked = enforced and #addonVersions > 0 and not DoesClientVersionMatch()

-- The flavor check runs separately from the interface-version check: a build for one game lineage must
-- never run on another even when both report the same interface number. The manifest decides what
-- "matching" means for its lineage; a manifest without a detector never blocks.
local flavorEnforced = C_AddOns.GetAddOnMetadata(addonName, "X-FlavorCheck") == "enabled"
local flavorBlocked = flavorEnforced and TRB.Flavor ~= nil and type(TRB.Flavor.IsClientMatch) == "function" and TRB.Flavor.IsClientMatch() == false

local blocked = versionBlocked or flavorBlocked

---Whether this build targets a different game version or game flavor than the client it is running on.
---Every bootstrapping entry point checks this before touching settings, saved variables, or frames.
---@return boolean
...
if blocked then
	...
	local flavorName = L[TRB.Flavor.nameKey]
	...
	StaticPopupDialogs["TwintopResourceBar_VersionMismatch"] = {
		...
		OnShow = function(self)
			if flavorBlocked then
				self:SetFormattedText(L["FlavorGateMismatchPopupText"], addonVersion, flavorName)
			else
	...
	gateFrame:SetScript("OnEvent", function(_, event, arg1)
		if event == "ADDON_LOADED" then
			if arg1 == addonName then
				savedVariables = TRB.Flavor.GetSavedVariables()
			end
		elseif event == "PLAYER_LOGIN" then
			if flavorBlocked then
				print(string.format(L["FlavorGateMismatchChatMessage"], addonVersion, flavorName))
			else
		...
		elseif event == "PLAYER_LOGOUT" then
			TRB.Flavor.SetSavedVariables(savedVariables)
		end
	end)
end
