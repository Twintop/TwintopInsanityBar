...
---Hides or shows wrapper frames for form-dependent bars of a class whose display follows
---shapeshift forms (descriptor `forms`). When the form changes, bars like combo points
---(secondary) and mana may become invisible. If such a bar is the root of its own tree
---(screen-anchored), the wrapper should be hidden so it doesn't linger as an empty container or
...
function TRB.Functions.EditMode:RefreshFormWrapperVisibility(settings, forest)
	local descriptor = TRB.Functions.Character:GetActiveSpecDescriptor()
	local forms = descriptor and descriptor.forms
	if forms == nil or forms.IsBarEligibleForLayout == nil then
		return
	end
...
function TRB.Functions.EditMode:CalculateWrapperLayout(settings, includeHidden, rootBarKey)
	...
	-- e.g. non-Feral Druids using Feral's) lays out with those dimensions. This injection must
	-- happen BEFORE building the anchor forest so secondary has proper dimensions for tree
	-- traversal. Without this, secondary has nil barSettings (0 dimensions) and walkTree cannot
	-- reach any bars anchored to it (e.g., mana bar). The borrowing predicate is deliberately
	-- ignored here: the tree must stay traversable even while the bar itself is switched off.
	--
	-- Visibility handling (which bars should vs. shouldn't show based on form) is delegated to
	-- IsBarVisibleForLayout in the walkTree function, rather than stripping bars from
	-- treeSettings. This ensures the tree is fully traversable while hidden bars still collapse
	-- to 0 height in the bounding box.
	local treeSettings = settings
	if barGroups.secondary and not settings.comboPoints then
		treeSettings = TRB.Functions.Bar:ResolveSharedSecondarySettings(settings, true, true)
	end
...
function TRB.Functions.EditMode:RegisterAllTreeRoots()
	...
	-- A spec that borrows another spec's combo point settings (descriptor `secondary.sharedFrom`)
	-- builds its forest with those so the correct anchor config for secondary (e.g., barKey="screen")
	-- is seen.
	local layoutSettings = settings
	if barGroups.secondary then
		layoutSettings = TRB.Functions.Bar:ResolveSharedSecondarySettings(settings, true)
	end
