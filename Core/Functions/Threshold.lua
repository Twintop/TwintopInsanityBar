...
local function GetCustomThresholdTargetDecimals(classId, specId, barKey)
	-- A secondary resource with finer granularity than its node count (e.g. Soul Shards tracked in
	-- fragments) declares the decimals its thresholds may target through the spec descriptor.
	if barKey == "secondary" then
		local descriptor = TRB.Functions.Character:GetSpecDescriptor(classId, specId)
		local decimals = descriptor and descriptor.secondary and descriptor.secondary.thresholdDecimals
...
function TRB.Functions.Threshold:GetCustomThresholdTargetInfo(settings, barGroups, barTarget, classId, specId)
	...
	elseif baseKey == "secondary" then
		...
		if sub ~= nil then
			...
			-- compressedView is a per-spec secondary bar option (only specs that offer it carry the setting).
			local compressedView = settings.colors and settings.colors.comboPoints
