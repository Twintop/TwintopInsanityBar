...
---@param key string
---@return boolean
local function ClassClaimsOtherBar(classId, key)
	...
	for _, specEntry in ipairs(classEntry.specs) do
		local otherBars = specEntry.descriptor and specEntry.descriptor.otherBars
		if otherBars ~= nil then
			for _, claimed in ipairs(otherBars) do
				if claimed == key then
					return true
				end
			end
...
---Returns the Other Bars keys that apply to a scope, in tab order. A class-scoped bar (definition
---`classScoped`, e.g. Feign Death) only applies to classes that claim it through a spec descriptor and
---is deliberately absent from the global (nil classId) scope too: with one class able to use it, a
---global copy would just be a setting nobody could reach, so that class's specs own it outright.
---@param classId integer? # nil is the global (core) scope
---@return string[]
function TRB.Classes.BarTypeRegistry:GetOtherBarKeys(classId)
	-- Callers reach this through the class table, so resolve the singleton for the definitions.
	...
	for _, key in ipairs(self.otherBarKeys) do
		...
		if definition == nil or not definition.classScoped or ClassClaimsOtherBar(classId, key) then
			keys[#keys + 1] = key
		end
	end
	return keys
...
---Registers the bar types every flavor shares: the mana and utility bars, the cast bars, the global
---cooldown bar and the mirror timers. Class bar types (Stagger, Holy Words, Bone Shield, ...) are
---registered by the owning class module (Flavors\<Flavor>\ClassModules\BarTypes\), which loads after
---Core but before anything constructs bars.
function TRB.Classes.BarTypeRegistry:RegisterBuiltInTypes()
	local L = TRB.Localization or {}

	-- Mana bar (any spec whose primary resource is not mana but that still tracks mana)
	...
	-- Feign Death is class-scoped: only classes whose spec descriptors claim it (otherBars) get it, and
	-- its options tab is only built for those specs.
	for _, mt in ipairs({
		{ key = "fatigue", name = L["ResourceFatigue"] },
		{ key = "breath", name = L["ResourceBreath"] },
		{ key = "feignDeath", name = L["ResourceFeignDeath"], classScoped = true },
	}) do
		self:Register(TRB.Classes.BarTypeDefinition:New({
			key = mt.key,
			displayName = mt.name,
