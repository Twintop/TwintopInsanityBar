	...
	if config.useApplyLayout then
		...
		if singleNode then
			...
			if nodeFrame then
				...
				-- Uninterruptible shield: cast-bar-only (only present on cast bar icon settings). Laid out
				...
				local shield = groupSettings and groupSettings.icon and groupSettings.icon.uninterruptibleShield
				if shield then
					singleNode:ApplyShieldLayout(shield.mode or "behind", shield.target or "icon", shield.sizePercent or 125, shield.anchor or "CENTER")
				end
			end
...
function TRB.Functions.Bar:ApplyAnchoredBarGroupLayout(settings, barGroups, barKey, anchorGroup, applyAppearance)
	...
	if barKey == "secondary" then
		...
		self:ConstructSecondaryBarGroup(settings, anchorGroup, barGroup, applyAppearance, secondaryMinMaxMode)
