...
-- Mainline saved-variable migrations. This is the historical record of every settings shape the
-- retail addon has shipped, applied in order on every login (each step is idempotent). It is frozen:
-- add new steps at the end, never edit old ones. Core calls TRB.Flavor.PortForwardSettings through
-- TRB.Functions.Settings:PortForwardSettings; a flavor with no history simply does not define it.

---Seeds settings.manualUpdateChecks for a fresh settings table. Each entry is a one-shot per-class flag
---that the class module checks at load to run a manual migration once.
---@return table
function TRB.Flavor.DefaultManualUpdateChecks()
	return {
...
---Called by Core after a class has just received fresh default bar text (a spec the user never
---initialized). Marks the Midnight bar text reset as done for that class so the one-shot migration
---meant for users upgrading from TWW never clobbers text the user edits before first logging into it.
---@param settings table
---@param className string
function TRB.Flavor.OnBarTextSeeded(settings, className)
	if settings.manualUpdateChecks
...
---@param className string
function TRB.Flavor.ShowMidnightBarTextResetMessage(className)
	print(string.format(L["MidnightBarTextResetMessage"], className))
end

---Applies every historical settings migration to a saved-variables shaped table.
---@param settings table? # Defaults to the live saved variables
function TRB.Flavor.PortForwardSettings(settings)
	---@diagnostic disable-next-line: unused-local
	local TwintopInsanityBarSettings = settings or TRB.Flavor.GetSavedVariables()
...
---the saved variables were merged. Reports what it did so the class module can react (e.g. mark bar text
---entries it seeds as already present).
---@param settings table
---@param classEntry TRB.Data.ClassRegistryEntry
---@return table checks # barTextReset = true when every spec of the class had its bar text reset to defaults
function TRB.Flavor.RunManualUpdateChecks(settings, classEntry)
	local checks = {}
	local className = classEntry.className
	local resets = settings.manualUpdateChecks and settings.manualUpdateChecks.midnightBarTextReset
	if resets ~= nil and resets[className] ~= true then
		-- Midnight changed the bar text variable set; users upgrading from The War Within get their bar
		-- text reset to the new defaults once per class.
		...
		for _, specEntry in ipairs(classEntry.specs) do
			...
			if loader ~= nil then
				settings[className][specEntry.specName].displayText.barText = loader()
			end
		end
		resets[className] = true
		TRB.Flavor.ShowMidnightBarTextResetMessage(L[classEntry.classModuleName])
		checks.barTextReset = true
	end
	return checks
end
