...
do
	...
	-- Druid display follows the shapeshift form: cat form shows Feral's combo points, bear form Guardian's
	-- rage, moonkin Balance's astral power, everything else the mana bar. Core asks these hooks instead of
	-- knowing any of that.
	local FERAL, GUARDIAN, BALANCE, RESTORATION = 2, 3, 1, 4

	---Which spec's settings the current form displays, honoring the spec's enableFormSwitching option.
	---@param specId integer
	---@param formName string
	---@param activeSpecSettings table?
	---@return integer
	local function GetDisplaySpecId(specId, formName, activeSpecSettings)
		local displayBar = activeSpecSettings and activeSpecSettings.displayBar
		if displayBar and displayBar.enableFormSwitching == false then
			return specId
		end
		...
		elseif formName == "moonkin" then
			return specId == BALANCE and BALANCE or RESTORATION
		end
		return RESTORATION
	end

	---Whether a form-dependent bar is eligible to show right now. Secondary (combo points) is native in
	---cat form; in other forms the showComboPoints checkbox decides (defaults on for Feral, off otherwise).
	---The mana bar shows only for Balance while the primary bar shows Astral Power.
	---@param barKey string
	---@param activeSpecSettings table?
	---@return boolean?
	local function IsBarEligibleForLayout(barKey, activeSpecSettings)
		if barKey ~= "secondary" and barKey ~= "mana" then
			return nil
		end
		local specId = TRB.Data.character.specId
		local currentForm = TRB.Data.character.currentShapeshiftForm or "humanoid"
		local displaySpecId = GetDisplaySpecId(specId, currentForm, activeSpecSettings)
		local displayBar = activeSpecSettings and activeSpecSettings.displayBar
		if barKey == "secondary" then
			if displaySpecId == FERAL then
				return true
			elseif specId == FERAL then
				return not displayBar or displayBar.showComboPoints ~= false
			else
				return displayBar ~= nil and displayBar.showComboPoints == true
			end
		end
		return specId == BALANCE and displaySpecId == BALANCE
	end

	---Non-Feral specs borrow Feral's combo point configuration while form switching or showComboPoints is on.
	---@param activeSpecSettings table?
	---@return boolean
	local function IsSharedSecondaryActive(activeSpecSettings)
		local displayBar = activeSpecSettings and activeSpecSettings.displayBar
		local enableFormSwitching = not (displayBar and displayBar.enableFormSwitching == false)
		local showComboPoints = displayBar ~= nil and displayBar.showComboPoints == true
		return enableFormSwitching or showComboPoints
	end

	SpecDescriptor:DeclareForClass("druid", {
		forms = {
			IsBarEligibleForLayout = IsBarEligibleForLayout,
			exportDisplayBarKeys = { "enableFormSwitching", "showComboPoints" },
		},
	})
	for _, specName in ipairs({ "balance", "guardian", "restoration" }) do
		SpecDescriptor:Declare("druid_" .. specName, {
			secondary = { sharedFrom = "druid_feral", IsSharedActive = IsSharedSecondaryActive },
		})
	end

	SpecDescriptor:Declare("druid_balance", {
		manaBar = true,
		customBars = { "mana" },
		barTextAnchorFrames = {
