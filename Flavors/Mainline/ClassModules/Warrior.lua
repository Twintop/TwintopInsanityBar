...
TRB.Functions.Bootstrap:RegisterClassModule({
	...
	fillSpellData = { FillSpellData_Arms, FillSpellData_Fury, FillSpellData_Protection },
	---Seeds the Whirlwind charge bar text entries into existing Fury users' lists once.
	OnSettingsLoaded = function(settings, savedVariables, checks)
		if checks.freshInstall then
			return
		end
		local fury = settings.warrior.fury
		if fury.displayText.migrations == nil then
			fury.displayText.migrations = {}
		end
		if not fury.displayText.migrations.whirlwindBarTextSeeded then
			...
			for _, entry in ipairs(whirlwindBarTextEntries) do
				table.insert(fury.displayText.barText, entry)
			end
			fury.displayText.migrations.whirlwindBarTextSeeded = true
		end
	end,
})
