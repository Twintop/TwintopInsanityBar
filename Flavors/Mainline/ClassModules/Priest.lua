...
TRB.Functions.Bootstrap:RegisterClassModule({
	...
	fillSpellData = {}, -- every spec fills its spell data inside SwitchSpec
	---Bar text entries the Priest specs gained over time are seeded into existing users' lists once each;
	---a full bar text reset already contains them, so it marks them as seeded first.
	OnSettingsLoaded = function(settings, savedVariables, checks)
		if checks.freshInstall then
			return
		end
		local priest = settings.priest
		if checks.barTextReset then
			-- Mark Holy Word bar text as seeded since HolyLoadDefaultBarTextSettings includes them
			priest.holy.displayText.migrations = priest.holy.displayText.migrations or {}
			priest.holy.displayText.migrations.holyWordBarTextSeeded = true
			-- Mark Lightweaver bar text as seeded since HolyLoadDefaultBarTextSettings includes them
			priest.holy.displayText.migrations.lightweaverBarTextSeeded = true
			-- Mark Power Word bar text as seeded since DisciplineLoadDefaultBarTextSettings includes them
			priest.discipline.displayText.migrations = priest.discipline.displayText.migrations or {}
			priest.discipline.displayText.migrations.powerWordBarTextSeeded = true
			-- Mark Angelic Feather bar text as seeded since all specs' LoadDefaultBarTextSettings include them
			priest.discipline.displayText.migrations.angelicFeatherBarTextSeeded = true
			priest.holy.displayText.migrations.angelicFeatherBarTextSeeded = true
			priest.shadow.displayText.migrations = priest.shadow.displayText.migrations or {}
			priest.shadow.displayText.migrations.angelicFeatherBarTextSeeded = true
		end

		-- Seed Holy Word bar text entries for existing users who don't have them yet
		priest.holy.displayText.migrations = priest.holy.displayText.migrations or {}
		if priest.holy.displayText.migrations.holyWordBarTextSeeded ~= true then
			local extraEntries = TRB.Options.Priest.HolyLoadHolyWordBarTextSettings()
			for _, v in ipairs(extraEntries) do
				table.insert(priest.holy.displayText.barText, v)
			end
			priest.holy.displayText.migrations.holyWordBarTextSeeded = true
		end

		-- Seed Lightweaver bar text entries for existing users who don't have them yet
		if priest.holy.displayText.migrations.lightweaverBarTextSeeded ~= true then
		...
		priest.discipline.displayText.migrations = priest.discipline.displayText.migrations or {}
		if priest.discipline.displayText.migrations.powerWordBarTextSeeded ~= true then
			local extraEntries = TRB.Options.Priest.DisciplineLoadPowerWordBarTextSettings()
			for _, v in ipairs(extraEntries) do
				table.insert(priest.discipline.displayText.barText, v)
			end
			priest.discipline.displayText.migrations.powerWordBarTextSeeded = true
		end

		-- Seed Angelic Feather bar text entries for existing users who don't have them yet (all 3 specs)
		for _, specName in ipairs({ "discipline", "holy", "shadow" }) do
			priest[specName].displayText.migrations = priest[specName].displayText.migrations or {}
			if priest[specName].displayText.migrations.angelicFeatherBarTextSeeded ~= true then
				local afEntries = TRB.Options.Priest.LoadAngelicFeatherBarTextSettings()
				for _, v in ipairs(afEntries) do
					table.insert(priest[specName].displayText.barText, v)
				end
				priest[specName].displayText.migrations.angelicFeatherBarTextSeeded = true
			end
		end
	end,
})
