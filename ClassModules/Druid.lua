...
---@diagnostic disable-next-line: missing-parameter
	...
	local function GetBerserkRemainingTime()
		...
		elseif specId == 3 then
			if talents:IsTalentActive(TRB.Data.spells.incarnationGuardianOfUrsoc) then
...
---@diagnostic disable-next-line: cast-local-type
	...
	local function UpdateSnapshot_Guardian()
		...
		if talents:IsTalentActive(spells.afterTheWildfire) then
			snapshotData.snapshots[spells.afterTheWildfire.id].cooldown:Refresh()
		end
		
		if talents:IsTalentActive(spells.frenziedRegeneration) then
