...
local specCache = {
	...
	guardian = TRB.Classes.SpecCache:New() --[[@as TRB.Classes.SpecCache]],
...
---@diagnostic disable-next-line: cast-local-type
	spells = specCache.guardian.spellsData.spells --[[@as TRB.Classes.Druid.GuardianSpells]]
	...
	---@type TRB.Classes.Snapshot
	specCache.guardian.snapshotData.snapshots[spells.rebirth.id] = TRB.Classes.Snapshot:New(spells.rebirth)
	---@type TRB.Classes.Snapshot
	specCache.guardian.snapshotData.snapshots[spells.ironfur.id] = TRB.Classes.Snapshot:New(spells.ironfur)
	---@type TRB.Classes.Snapshot
	...
	---@type TRB.Classes.Snapshot
	specCache.guardian.snapshotData.snapshots[spells.afterTheWildfire.id] = TRB.Classes.Snapshot:New(spells.afterTheWildfire, nil, true)
	specCache.guardian.snapshotData.snapshots[spells.afterTheWildfire.id].buff:SetCustomProperties({
		{
			name = "currentRage",
			dataType = "number",
			index = 16,
	...
	---@type TRB.Classes.Snapshot
	specCache.guardian.snapshotData.snapshots[spells.toothAndClaw.id] = TRB.Classes.Snapshot:New(spells.toothAndClaw)
	...
	---@diagnostic disable-next-line: cast-local-type
	spells = specCache.restoration.spellsData.spells --[[@as TRB.Classes.Druid.RestorationSpells]]
...
local function GetBerserkRemainingTime()
	...
	elseif specId == 3 then
		...
		if talents:IsTalentActive(spells.incarnationGuardianOfUrsoc) then
...
local function UpdateSnapshot_Guardian()
	...
	snapshotData.snapshots[spells.rebirth.id].cooldown:Refresh()
						...
						if spell.isSnowflake then -- These are special snowflakes that we need to handle manually
							if spell.settingKey == spells.starsurge.settingKey then
								...
								if showThreshold then
									if snapshots[spells.starweaversWeft.id].buff.isActive and specSettings.audio.starweaversReady.enabled and snapshotData.audio.playedstarweaverCue == false then
							...
							elseif spell.id == spells.starfall.id then
								...
								if showThreshold then
									if snapshots[spells.starweaversWarp.id].buff.isActive and specSettings.audio.starweaversReady.enabled and snapshotData.audio.playedstarweaverCue == false then
										snapshotData.audio.playedstarweaverCue = true
										snapshotData.audio.playedSfCue = true
										PlaySoundFile(specSettings.audio.starweaverProc.sound, coreSettings.audio.channel.channel)
			...
			if specSettings.displayBar.neverShow == false then
				...
				for _, v in pairs(spells) do
					...
					if (spell:Is("TRB.Classes.SpellThreshold") or spell:Is("TRB.Classes.SpellComboPointThreshold")) and spell:IsValid() then
						...
						if spell.isSnowflake then -- These are special snowflakes that we need to handle manually
							if spell.id == spells.maul.id or spell.id == spells.raze.id then
								if snapshots[spells.toothAndClaw.id].buff.isActive == true then
...
barContainerFrame:SetScript("OnEvent", function(self, event, ...)
	...
	if event == "COMBAT_LOG_EVENT_UNFILTERED" then
		...
		if entry.sourceGuid == TRB.Data.character.guid then
			...
			elseif specId == 3 and TRB.Data.barConstructedForSpec == "guardian" then
				if entry.spellId == spells.rebirth.id then
					if entry.type == "SPELL_CAST_SUCCESS" then
						snapshotData.snapshots[entry.spellId].cooldown:Initialize()
					end
				...
				elseif entry.spellId == spells.toothAndClaw.id then
					snapshotData.snapshots[spells.toothAndClaw.id].buff:Initialize(entry.type)
				end
